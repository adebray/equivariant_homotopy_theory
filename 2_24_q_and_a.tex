...
categorical fixed points will send it to $S^{-V^H}$, whereas categorical fixed points will look more like a tom
Dieck splitting.
...
Tate spectra fit into this story: the Tate spectrum $X^{tC}$ of $X$ fit into a homotopy pullback diagram where the
other three corners are the three kinds of fixed points. \TODO: the right diagram. This looks like an arithmetic
square, and this analogy can be made precise.
\begin{ques}
...
On the subject of mathematical literature, the HULK smash product \citeme{Mandell} is possibly the best
mathematical joke of all time.
\begin{ques}
You've said that Hill-Hopkins-Ravenel have significantly affected the direction of equivariant stable homotopy
theory. Can you elaborate on this?
\end{ques}
...
\end{ques}
For $G$ a finite group, everything is the same: you can use $\Gamma$-$G$-spaces or $G$-$\Gamma$-spaces (which are
different notions for the same thing); you can use functors from $\Gamma$ to $G$-spaces, and a few other ones.
Then, loop spaces come from a $\E_\infty$-operad action; for technical reasons you may need to assume there are
trivial representations in the universe.
...
proof.\footnote{\TODO: I should go fix that part of the notes.}
\begin{ques}
...
of finite groups also uses this material. Goodwillie calculus also uses equivariant methods: Nick Kuhn proved that
the vanishing\footnote{\TODO: I think this wasn't the right word\dots} of the Taylor tower is equivalent to the
vanishing of the Tate spectrum.
