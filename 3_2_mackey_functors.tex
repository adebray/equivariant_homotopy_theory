...
\Tr_H^G\Res_K^G = \bigoplus_{x\in K\backslash G/H} \Res_{H\cap x^{-1}Kx}^H\Tr_{K\cap x^{-1}Hx}^Kc_x,
\end{equation}
where $c_x$ is the conjuation map. \TODO: not sure whether the final formula is correct.
\begin{rem}
...
functors are sketched or left out in the literature, but \citeme{Webb, Devinaz} are pretty good references.
\end{rem}
