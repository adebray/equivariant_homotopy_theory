Today, we're going to discuss the homotopy theory of spectra in the nonequivariant case. Recall that we started
with $\fD$-spaces, where $\fD$ is a small \term{topological category} (meaning $\Top$-enriched), and that the
...
\begin{itemize}
	\item $f$ is a \term{level equivalence} if for all $d\in\fD$, $f(d)\colon X(d)\stackrel\simeq\to Y(d)$ is a
	weak equivalence.
...
retracts of \term{cellular objects}, which are built by iterated pullbacks
\[\xymatrix{
...
\begin{thm}
There is a \term{stable model structure} where the weak equivalences are stable equivalences. For $\Sp^\N$ and
...
Another interesting fact is that if $X$ and $Y$ are $\Omega$-prespectra and $f\colon X\to Y$ is a level
equivalence, then $f$ is a $\pi_*$-isomorphism. This is an example of \term{Bousfield localization}. Let $\fC$ be a
...
\begin{defn}
Let $\fC$ be a topologically enriched model category and $S$ be as above. Then, an \term{$S$-local object} in $\fC$
...
A map $f\colon X\to Y$ is an \term{$S$-local equivalence} if for all $S$-local objects $Z$, the induced map
...
where the weak equivalences are \term{positive equivalences} (so those which are detected by \term{positive
$\Omega$-spectra}, i.e.\ those where $X_0\to\Omega X_1$ need not be an equivalence). But this still doesn't behave
...
\term{connective spectra}, i.e.\ those whose negative homotopy groups vanish.
\end{exm}
