...
\begin{comp}{rem}{enumerate}
	...
	Thus, we don't expect it to commute with colimits in general. However, it does with with many important ones,
	as in the following proposition.\qedhere
\end{comp}
\begin{prop}
...
be a $\theta$-connected $G$-map. Given $g:A\to Y$, $h:A\times I\to Z$., and $f:X\to Z$ such that $eg = hi_0$ and
$fi = hi_1$, there exist maps $X\to Y$ and $X\times I\to Z$ that make the following diagram commute:
\[\text{[diagram to be made later]}.\]
% TODO: get the diagram from May Concice Course ch. 11 or 6; or equivariant version from Alaska notes!
\end{thm}
