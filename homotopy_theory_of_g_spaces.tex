...
\begin{ex}
There is a group action on the torus $S^1 \times S^1$ given by $z(z_1, z_2) = (zz_1, z_2)$. With this group action, the torus has the $S^1$ CW Complex structure with one zero cell $S^1/e \times *,$, and a single one cell $S^1/e \times [0,1]$, where the attaching map sends $0$ and $1$ to $*$. This is a specific example that $n$ cells on $G$ CW complex structures may not be "n-dimensional" in the traditional sense.  
\end{ex}
