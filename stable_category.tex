...
\begin{defn}
The \term{$S$-category} is the category whose objects are the objects in $\Top$ and whose morphisms are
...
\begin{itemize}
	...
	\term{flip map} $\tau:X\wedge Y\isom Y\wedge X$.
...
\begin{defn}
The \term{evaluation map} is the unit $X\wedge F(X,Y)\to Y$, and the \term{coevaluation map} is $X\to F(Y,Y\wedge
X)$. The \term{dual} of $X$ is $DX = F(X,S)$.
...
There are a few good references for this: Dold-Puppe is one, and~\cite{LMS} is another, though it presents a
...
$\eta\colon X\wedge Y\to S$ and $\eta: S\to X\wedge Y$ such that the compositions
\[\xymatrix{
...
spheres by way of the \term{Pontrjagin-Thom map} $S^n\to \R^n/(\R^n\setminus M_\e)$ (heuristically, crushing
everything outside $M_\e$), and $\R^n/(\R^n\setminus M_\e)\cong D(V)/S(V)$, the \term{Thom space} $T_\nu$ of the
normal bundle to $M$.
