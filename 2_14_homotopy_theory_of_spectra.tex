...
the stable model category.\index{level model structure}\index{$\infty$-category!of spectra}}
...
equivalences are weak equivalences.\index{stable model structure}\index{Omega-prespectra@$\Omega$-prespectra}
\begin{prop}
...
weak equivalences are \term{positive equivalences} (so those which are detected by \term{positive
...
$\Sigma^\infty$ be left adjoint to evaluation at $0$, and it's okay, if not perfect.\index{Eilenberg-Mac Lane
space}
\begin{rem}
