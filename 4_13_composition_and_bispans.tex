Recall that we've been discussing the category $\Bispan_\fC(X,Y)$ of bispans
\begin{equation}
\label{span1}
\xymatrix{
...
Y]$. We start by superimposing the two diagrams as in~\eqref{snake}, and then let $B'\coloneqq B\times_Y C$ and
...
Now, we can define the \term{composition} of these two bispans to be the bispan $[X\gets \widetilde A\to\Pi_fB'\to
Z]$.
...
The second step of the proof is analogous, composing with $T_h$, is analogous.
\end{proof}
\begin{prop}\hfill
\label{NTRcomp}
\begin{enumerate}
...
Explicitly, we want to compose $X\stackrel\gets\id X\stackrel{g'}\to Y\stackrel\id\to Y$ and $X\stackrel\gets\id
...
as they could be.
\begin{rem}
This category of bispans is reminiscent of the Dwyer-Kan hammock localization, and it may be possible to obtain
bispans from the hammock localization of a particular category with weak equivalences. Is this useful? Maybe.
\end{rem}
...
\begin{comp}{rem}{enumerate}
	...
	analogy.
% "the algebra of Tambara functors is still under construction" TODO
% nakaoka: algebra of Tambara functors, 3 or 4 papers. very recent, closely related to project of equivariant DAG ~
% Tambara functor AG. how do you localize? what happens when you localize? interesting and complicated. I suspect
% strongly there are algebraic questions easy to find, pose, and answer becausenot very many people are looking.
...
\begin{enumerate}
	...
	the norm is the \term{Evans multiplicative transfer}, which we'll talk about next time.
...
\end{exm}
\begin{prop}
$P_\fD^G$ is a category if $\fD$ is \term{stable under pullback}, i.e.\ for any pullback diagram
\[\xymatrix{
...
\begin{defn}
Let $\fD$ be a wide, pullback stable, coproduct complete subcategory of $\fC$. Then, a \term{$\fD$-Tambara functor}
is a functor $P_\fD^G\to\Ab$. You might call these ``incomplete Tambara functors.''
\end{defn}
