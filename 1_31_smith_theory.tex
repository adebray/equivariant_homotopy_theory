...
\end{warn}
The finiteness in \cref{smith} is key: Elmendorf's theorem lets us build a $\Z/p$-complex with non-equivariant
...
\Z/p$, whose orbit category is simple:\index{orbit category!of $\Z/p$}
\[\xymatrix{
...
are not homeomorphic, but produce homotopy equivalent models of $B\Z/p$.} and give it a $\Z/p$-action by $z\mapsto
e^{2\pi i/p}z$. Then, the infinite-dimensional lens space is $S^\infty/\Z/p$.\index{classifying space!for $\Z/p$}

The quotient $S^\infty\surj S^\infty/\Z/p$ is a covering map, and $S^\infty$ is contractible. The proof goes through
...
\end{itemize}
Thus $S^\infty/\Z/p$ is the quotient of a contractible space by a free $\Z/p$-action, so it deserves to be called
$B\Z/p$.
