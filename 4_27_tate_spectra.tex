Today (and possibly next lecture): Tate spectra. Next week, we'll talk about the slice spectral sequence and future
research directions.
...
level.
\end{rem}
Let $M$ be a $k[G]$-module. Then, the \term{algebraic orbits} are $M_G\coloneqq k\otimes_{k[G]} M$, and the
...
We'll think of $N$ as a map $H_0(G;M)\to H^0(G;M)$, which ``sews together'' group homology and group cohomoloy with
...
\end{equation}
\end{subequations}
You can build this by forming \term{complete resolutions}: beginning with a pair $P_*\to k$ and $k\to I_*$,
...
\begin{defn}
Using this, define the \term{Tate cohomology} $\tH^*(G;M)$ is the cohomology of the complex
...
structure. Ongoing work of~\citeme{Blumberg-Mandell} is trying to resolve this.} Namely, let $\set{E_r}$ be a CW
