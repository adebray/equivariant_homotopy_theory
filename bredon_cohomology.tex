...
Today I was running about two minutes late. Sorry, everybody! (Group: That's ok. We actually waited to start until you got there anyway. In the meantime, Andrew talked about a "Q and A" session some weeknight (with mention of pizza) next week (the week of 1/30/2017) and asked anyone interested in attending to email him what nights work for you.)
...
\begin{itemize}
	...
	\item The \term{Bredon homology} is
	\[H_n^G(X; M) \coloneqq H_n(\underline C_n(X)\otimes_{\sO_G} M).\]
	By this tensor product, we mean a coend:
	\[\underline C_*(X)\otimes_{\sO_G} M = \coprod_{G/H} \underline C_n(X)(G/H)\times M(G/H)/\sim,\]
...
So a coefficient system is determined by a map $G_1\to G_2$ and an involution on $G_2$. In our case, $\underline
...
\end{rem}
\begin{ex}
We constructed a functor $\underline H:\Fun(\sO_G\op,\Ab)\to \Fun(\sO_G\op, \Top)$. Show that $\underline HM$
