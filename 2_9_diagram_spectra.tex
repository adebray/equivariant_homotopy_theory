...
Boardman's are rare in the literature.\index{S-category@$S$-category}
\end{rem}
\begin{defn}
...
basepoint. $\fD$-spaces are called \term*{$\Gamma$-spaces}, and agree with Segal's
notion of $\Gamma$-spaces, which are defined differently. The multiplication comes from the map $\psi\colon 2_+\to
1_+$ sending $1,2\mapsto 1$.\index{Gamma-space@$\Gamma$-space}
...
\begin{defn}
A prespectrum is an \term[Omega-prespectrum@$\Omega$-prespectrum]{$\Omega$-prespectrum} if for all $n$,
...
are the maps $F_d(S_+^{n-1}\to D_+^n)$ for each $n\ge 1$ and $d\in\fD$, and the \term[acyclic cofibrations!for the
level model structure]{acyclic cofibrations} are $F_d(D_+^n\to (D_n\times I)_+)$ for each $n\ge 1$ and $d\in\fD$.
...
\begin{defn}
A map of orthogonal spectra $f\colon X\to Y$ is a \term[pistar-isomorphsim@$\pi_*$-isomorphism!of orthogonal
spectra]{$\pi_*$-isomorphism} if $Uf\colon UX\to UY$ is a $\pi_*$-isomorphism of prespectra.
\end{defn}
...
\begin{defn}
Similarly, we define an \term[Omega-spectrum@$\Omega$-spectrum!for orthogonal spectra]{$\Omega$-spectrum} in
orthogonal spectra to be an orthogonal spectrum $X$ such that the adjoints to the structure maps
$X(U)\congto\Omega^V X(U\oplus V)$ are homeomorphisms.
\end{defn}
