...
\end{exm}
In the equivariant case, there's even more structure, and notions of ``extra special'' $\Gamma$-spaces.
\begin{defn}
...
constructed above, then the \term{generating cofibrations} are the maps $F_d(S_+^{n-1}\to D_+^n)$ for each $n\ge 1$
and $d\in\fD$, and the \term{acyclic cofibrations} are $F_d(D_+^n\to (D_n\times I)_+)$ for each $n\ge 1$ and
$d\in\fD$.
