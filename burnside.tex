...
So if $H\subset G$, we'd like to build a transfer mp $H^*(H;M)\to H^*(G;M)$, where $M$ is a $G$-module made an
...
\begin{rem}
Historically, Bordman constructed the stable homotopy category as a formal completion of the $S$-category. Then,
people tried to find ``point-set models,'' stable model categories whose homotopy categories are isomorphic to
Bordman's category. There are several options, but explicit proofs that their homotopy categories are equivalent to
Bordman's are rare in the literature.
\end{rem}
\begin{defn}
...
[m+n]$. Then, there is a monoid $S$ in $\N$-spaces defined by $[n]\mapsto S^n$. The category of $S$-modules is
...
\begin{defn}
A prespectrum is an \term{$\Omega$-prespectrum} if for all $n$, $X_n\stackrel\cong\to\Omega^mX_{m+n}$.
\end{defn}
\begin{defn}
