...
\begin{itemize}
	...
	maps $G\to X$, with $G$-action $(gf)(g') = f(g'g)$. This is called the \term{coinduced $G$-action} on
	$\Map^H(G,X)$.\footnote{This actually \emph{is} a group action, since if $a,b,g\in G$, then $(a(b f))(g) =
	(bf)(ga) = f(gab) = (ab(f))(g)$.} Sometimes this is also denoted $F_H(G, X)$.\qedhere
\end{itemize}
\end{exm}
...
usual spheres; thus, these $S^V$ are called \term[index representation sphere]{representation spheres}.
\end{exm}
...
the same data as a path in $G\Map(X,Y)$. A \term{$G$-homotopy equivalence} between $X$ and $Y$ is a map $f\colon
X\to Y$ such that there exists a $g\colon Y\to X$ and $G$-homotopies $gf\sim \id_X$ and $fg\sim\id_Y$.
\end{defn}
