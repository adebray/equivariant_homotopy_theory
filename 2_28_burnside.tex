...
functors are $\Ab$-valued presheaves on the Burnside category. We'll begin discussing this in this section, and
provide another deifnition for the Burnside category which is more hands-on.
...
\end{defn}
\begin{rem}
If your category number\footnote{Freed~\cite{CSRemarks} defines the \term{category number} of a mathematician to be
...
\term{Green functors}~\cite{Green}. These aren't quite the right objects, however: we need to account for stable
...
data; these are called \term{Tambara functors} or \term{TNR functors}~\cite{Tambara}.
