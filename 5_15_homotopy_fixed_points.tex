...
\begin{thm}
\label{HFPSS}
If $X$ is a $G$-spectrum, there's a spectral sequence, called the \term{homotopy fixed-point spectral
...
\end{rem}
\TODO: the cofiber sequence $X_{hG}\to H^{hG}\to X^{tG}$ induces a morphism of multiplicative spectral sequences
...
$C_2$-spectrum is called \term{$\KR$-theory}, and denoted $\KR$. Its homotopy fixed points are real $K$-theory:
\begin{thm}
$\KR^{hC_2} = \KO$.
\end{thm}
