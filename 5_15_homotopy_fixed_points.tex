...
\[E^2_{p,q} = H^{-p}(G, \pi_q(X))\Longrightarrow \pi_{p+q}(X^{hG}).\]
\end{thm}
\begin{exm}
...
\[E^2_{p,q} = H_p(G; \pi_q(Hk)) = \begin{cases}
	H_p(G; k), &q = 0\\
	0, &\text{otherwise.}
\end{cases}\]
...
we just have to compute $H_p(G;k)$ for $k\ge 0$. For example, if $G = \Z/2$ and $k = \F_2$, then $H_*(\Z/2; \F_2) =
H_*(\RP^\infty;\F_2)$, which is $\F_2$ in every nonnegative degree.

In general, there are no extension issues, since there's only one nonzero term in each total degree, and we
conclude
\[\pi_p(Hk^{hG}) = H_p(G; k).\]
If you let $G = \Z/2$ and $k$ be any field of odd characteristic, then $H_*(\Z/2; k) = k$ in degree $0$, and $0$
otherwise, so the homotopy groups of $Hk^{h\Z/2}$ are all trivial except for $\pi_0$, which is $k$.
\end{exm}
\begin{rem}
...
\begin{align*}
E^2_{p,q} &= H^{-p}(G, \pi_q(X))\Longrightarrow \pi_{p+q}(X^{hG})\\
E^2_{p,q} &= \widehat H^{-p}(G; \pi_q(X)) \Longrightarrow \pi_{p+q}(X^{tG}).
...
E^2_{p,q} &= H_p(G, \pi_q(X))\Longrightarrow \pi_{p+q}(X_{hG})
\end{align*}
...
homotopy orbit spectral sequence fit into this?
\begin{exm}
\label{reflection}
\TODO: inconsistency in the grading in this example; work through it again carefully.
