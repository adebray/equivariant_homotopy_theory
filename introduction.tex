...
approach in order to avoid getting bogged down in $\infty$-categories; moreover, the notes are supposed to be
...
\begin{itemize}
	...
	\term{$k$-Hausdorff}, i.e.\ the diagonal map $\Delta\colon X\to X\times X$ is closed when $X\times X$ has the
	compactly generated topology. See~\cite{StricklandCGWH, RezkCGWH} for more details.}
\end{itemize}
\end{defn}
