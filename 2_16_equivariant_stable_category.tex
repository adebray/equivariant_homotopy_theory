...
There is always a free-forgetful adjunction $F\colon \fC\rightleftarrows \fC[M]: U$, which has to do with the
...
philosophy in the proof of Theorem~\ref{Wirthiso}.
\begin{defn}
...
Following Example~\ref{orthogonal_HA}, we can define an \term{equivariant Eilenberg-Mac Lane spectrum} $HM$ to be
the orthogonal $G$-spectrum that assigns to a representation $V$ the $M$-linearization $HM(V)\coloneqq M[S^V]$.
Here, the orthogonal group acts through its action on $S^V$, and $G$ acts on both $M$ and $S^V$ from the right. The
