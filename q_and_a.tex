...
Y\to\Sph$ and $M:\Sph\to X\wedge Y$, such that the composition
\[\xymatrix{
...
the bar resolution to compute the derived tensor product: the functor $M:\sO_G\to\Top$ sending $G/H$ to the space
$G/H$ is thought of as a ``right $\sO_G$-module;'' instead of tensoring a bunch of elements togther, we get a bunch
of arrows, meaning we can replace with a coproduct. In some sense, it would be nice to take a tensor product with
$X$, but we have to do so in a derived sense, hence the bar construction.
...
Here, someone asked about the Fruedenthal suspension theorem, and this led to a digression.
\begin{rem}
...
same thing with, say, bordism rings of $BG$? % Strickland K(n)-local duality for finite groups and groupoids
\end{ques}
...
\end{ques}
Adams' book is interesting, with good calculations, but its construction of the stable category isn't so pleasant.
It might be better to consider Mandell-May-Schewe-Shipley's paper. It assumes you already know why you care, then
takes the historical constructions and shows how they all relate together. It may seem scary, but looking at the
constructions shows they're all not so bad.

Adams' infinite loop spaces book is good bedtime reading.
\begin{ques}
