...
$H_{C_2^n}^*(*; \underline{\Z/2})$.\index{motivic homotopy theory}
...
maps are wrong. Shouldn't restriction go from $M(C_2/C_2)\to M(C_2/e)$?}\index{Mackey functor}
\[\xymatrix{
...
\coloneqq H_{C_2}^{p+\sigma q}(\bl)$. The index $p$ is called the \term[fixed dimension!of a
$C_2$-representation]{fixed dimension}, and $q$ is called the \term[sign representation!of a
$C_2$-representation]{sign dimension}.
\begin{rem}
...
\end{proof}
Next we'll compute $H_{C_2}^{p,q}(*;\underline\Z)$. Let $S^{\sigma q}$ denote the representation sphere that's the
one-point compactification of $\R^{\sigma q}$ (i.e.\ $q$ copies of the sign representation); then, if $q >
0$,\index{representation sphere}
...
When $q > 0$, we use equivariant Spanier-Whitehead duality:\index{Spanier-Whitehead duality}
...
functor, denoted $\underline\Z\rop$, given by the diagram\index{opposite Mackey functor}
\[\xymatrix{
